...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num-envs", type=int, default=1,
                    help="number of envs")
    parser.add_argument("--double-dqn", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
                        help="double dqn")
...
class Trainer():
    ...
    def _loss(self, data: ReplayBufferSamples) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.double_dqn:
                target_max = self.policy.value(data.next_observations).gather(1, target_argmax.unsqueeze(1)).squeeze()
