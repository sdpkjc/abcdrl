...
class Trainer():
    def __init__(self, policy: Policy,
        ...
        self.optimizer_critic = optim.Adam(
            ...
            list(self.policy.critic_nn_1.parameters()), lr=self.learning_rate)

    def _loss(self, data: ReplayBufferSamples) -> torch.Tensor:
        global global_step
        with torch.no_grad():
            clipped_noise = (torch.randn_like(torch.Tensor(data.actions)) * self.policy_noise).clamp(
                -self.noise_clip, self.noise_clip
            )
            next_state_actions = (self.policy_t.action(data.next_observations) + clipped_noise.to(next(self.policy_t.parameters()).device)).clamp(
                self.policy.envs_single_action_space.low[0], self.policy.envs_single_action_space.high[0]
            )
