...
class Algorithm:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.optimizer_actor = optim.Adam(self.model.actor_nn.parameters(), lr=self.config["policy_lr"])
        ...
        if self.config["autotune"]:
            self.target_entropy = -torch.prod(
                torch.Tensor(get_space_shape(self.config["act_space"])).to(self.config["device"])
            ).item()
            self.log_alpha = torch.zeros(1, requires_grad=True, device=self.config["device"])
            self.alpha = self.log_alpha.exp().item()
            self.optimizer_a = optim.Adam([self.log_alpha], lr=self.config["q_lr"])
